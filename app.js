...
function submit(){if(!S||S.finished||!S.dartsInTurn.length)return;pushUndo();const p=S.players[S.active],ds=S.dartsInTurn.slice(),total=ds.reduce((a,d)=>a+dartValue(d),0);p.darts+=ds.length;p.turns++;p.total+=total;p.highestTurn=Math.max(p.highestTurn,total);if(isX())playX01(p,ds,total);else if(S.type==='cricket')playCricket(p,ds);else if(S.type==='shanghai')playShanghai(p,ds,total);else playTraining(p,ds,total);S.dartsInTurn=[];if(!S.finished)S.active=(S.active+1)%S.players.length;save();render()}
